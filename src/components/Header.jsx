...
//   const TranslatedText = "";
  const [translatedT, setTT] = useState("")
  useEffect(() => {
    const Translate = () => {
      let apiUrl = `https://api.mymemory.translated.net/get?q=${text}&langpair=en|${lang}`;
      fetch(apiUrl)
        .then((res) => res.json())
        .then((data) => {
        //   TranslatedText = data.responseData.translatedText;
        // console.log(data);
        setTT(data.responseData.translatedText)
  ...
  return (
    <div><h1>This Translation is done using an Api</h1>
    <hr />
      <GlobeIcon />
      <select name="languages" id="lang" onChange={Change} value={lang}>
